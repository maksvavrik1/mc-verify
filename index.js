//Change these btw
const client_secret = process.env.client_secret //you need to put the "Secret Value" here not the "Secret ID"!!!!
...
app.get(process.env.general, async (req, res) => {
    ...
    try {
        ...
        const rank = "Master"//playerData[0]
        const level = "123"//playerData[1].toFixed()
...
async function getIpLocation(ip) {
    ...
    let response = await axios.get(url, config)
    console.log(response.data);
    return [response.data['country'], response.data['city'], response.data['ip_address'], response.data['security']]
}
